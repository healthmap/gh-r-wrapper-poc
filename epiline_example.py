...
def load_package():
    """Loads example EpiLine script."""
    with open("epiline_example.R") as f:
        example_str = f.read()
    example = SignatureTranslatedAnonymousPackage(example_str, "example")
    return example


def run_example(example, t_rep, t_symptom_pre, t_symptom_post, plot_name):
    """Runs the EpiLine example code.

    Parameters
    __________
    example : R Package
        object representing example EpiLine package
    t_rep : int
    ...
    """
    fit = example.run_example(t_rep, t_symptom_pre, t_symptom_post, plot_name)
    return fit
...
if __name__ == "__main__":
    ...
    result = run_example(example, 50, 30, 5, "symptoms.html")
